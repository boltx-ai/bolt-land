...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const textToType =
    "The skilled-labor shortage is pushing manufacturers to their limits. BOLTX is a Physical AI company built to meet that moment. At our core is a multi-modal AI brain leveraging the latest advances in AI and robotics";
  ...
  function updateHeroGradient() {
    ...
    const scrollProgress = Math.max(
      0,
      Math.min(1, (windowHeight - heroRect.top) / (heroHeight + windowHeight)),
    );
    ...
    const interpolatedColor = {
      r: Math.round(
        startColor.r + (endColor.r - startColor.r) * (scrollProgress - 0.5),
      ),
      g: Math.round(
        startColor.g + (endColor.g - startColor.g) * (scrollProgress - 0.5),
      ),
      b: Math.round(
        startColor.b + (endColor.b - startColor.b) * (scrollProgress - 0.5),
      ),
    };
